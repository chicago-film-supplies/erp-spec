...
- ADR-0013 self-hosts on Linode. That decision named Caddy and the application; it did not name what
  else has to run beside them. These are that list.
- **Gotenberg** renders HTML to PDF and to screenshots
  ...
  documents all go through it. Template _content_ is git-canonical and Firestore is a rebuildable
...
- **The Victoria stack** is already the whole observability tier, pinned in
  ...
  victoria-traces v0.10.0, vmagent / vmalert / vmauth v1.148.0, alertmanager v0.33.1, node-exporter
  and cadvisor.
- **The application emits OTLP to an OpenTelemetry collector**
  (`otel/opentelemetry-collector-contrib` 0.157.0), which fans out to the three stores. The
  instrumentation is vendor-neutral; only the collector config knows what the backend is.
