...
- Trial balance, P&L and close are all periodised by **accounting date**. TigerBeetle timestamps
  are **posting time**.
...
- `.duckdb` files are gitignored and may be deleted at any time. Nothing may depend on one
  existing.
...
- SPIKE-007 decides how DuckDB is reached from Deno (native addon vs WASM), and whether
  duckdb-wasm in SolidJS gives client-side reporting for free.
