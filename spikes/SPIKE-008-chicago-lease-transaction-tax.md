...
### ⛔ Criterion 3 — nexus and rate-change handling: NOT DONE
...
⇒ **`in_progress`.** Nothing is blocked on infrastructure or on anyone else, but criterion 3 needs a
decision about what jurisdiction MEANS before it can be specified — and, per this spike's own note,
**a CPA should review the output. This spike produces the rules; it does not produce the
authority.**
