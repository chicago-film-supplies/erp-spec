...
- **Not vendored copies.** Each file is the CFS-specific half only: what upstream cannot know —
  ...
  reference material is cached _separately_, at `.claude/docs/<tool>.txt` (gitignored), refetched by
  `deno task fetch-llms-docs` when the local copy is over 24h old.
