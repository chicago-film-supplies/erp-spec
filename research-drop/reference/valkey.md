...
- **In-memory, never a source of truth.** Anything in Valkey must be rebuildable from Mongo / TB /
  Parquet — same discipline as the `.duckdb` cache ([[ADR-0017]]). It may never hold the ledger or be
...
`deno run` and `deno compile --self-extracting`, and the queue path is the **only** one of the
three npm dependencies that also survives `--bundle`, because it carries no native code.
...
- **Per-entity serialization needs no paid tier.** `SET NX PX` plus a compare-and-delete release is
  ...
  alternative and is not needed. GroupMQ was evaluated and rejected — last published 2025-11-26,
  255 days stale as of 2026-08-09.
