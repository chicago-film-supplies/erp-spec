...
- **Money stays integer minor units.** Use `z.int()` (or `z.bigint()` where the value can exceed
  `2^53`), never a plain `z.number()` float. Consider a branded type — `z.number().int().brand<"MinorUnits">()`
  — so dollars can never be assigned where cents are expected. Mirrors the TB u128 / BSON Long story.
- **Zod is the enforcement layer Mongo can't be.** The constructs `$jsonSchema` drops — discriminated
  unions (the order items tree), `if`/`then`, `unevaluatedProperties` — get enforced here at the
  boundary. The [[SPIKE-006]] rule: no construct unenforced in both DB and app.
