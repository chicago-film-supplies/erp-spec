...
> Corrected 2026-08-09. This note previously opened _"The read side ([[ADR-0006]])"_ and listed
...
> ADR-0006's artifact rule verbatim and drops its general "DuckDB is the read side" framing — which
> is precisely the sentence this note was repeating. `research-drop/` is invisible to `validate`,
> `gen` and `ingest` by design, so nothing here can ever go red on its own.
...
- `@duckdb/node-api@1.5.5-r.3`, reporting engine version `v1.5.5` from `SELECT version()` run under
  Deno (`code:2026-08-09:erp-spec@b555c5c:spikes/harness/probe-duckdb.ts`).
...
- **Keep money integer.** DuckDB has `DECIMAL`, `BIGINT`, `HUGEINT` — store minor units as
  `BIGINT`/`HUGEINT` to mirror TB's u128. Never introduce `DOUBLE`/floats on the read side. Verified
  round-tripping `2^127-1` through the Neo client without loss.
- **Performance does not discriminate between designs here.** ADR-0017 measured 999 invoices and
  ...
  access path; the interesting number is the _crossover_ at which the client-side WASM path stops
