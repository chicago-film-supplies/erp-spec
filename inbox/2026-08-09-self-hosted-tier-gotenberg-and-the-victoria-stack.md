...
both land on the host ADR-0013 provisions — so they are the second half of that decision rather than
new adoptions.
...
- The client deadline is **65s and deliberately exceeds Gotenberg's own ~60s render timeout** — a
  ...
  aborting below 60s recreated 502s that the infra fix had eliminated. A number with a reason, worth
...
out to the three Victoria stores.** The instrumentation is vendor-neutral; only the collector config
knows what the backend is. That is what makes the obs backend a swappable decision rather than a
rewrite, and it is the reason this can be adopted without a spike.
