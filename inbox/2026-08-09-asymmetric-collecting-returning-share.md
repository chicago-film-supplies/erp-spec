...
**And the contradiction dissolves.** HOT-002 recorded this share as suspicious because it seemed
to contradict "a confirmed deliver-out/customer-returns pattern". That pattern is
...
too small to justify a structural change on its own. Per-leg is justified by ADR-0011 instead: a
leg carries an actor, a clock-in and a `shift_id`, and labour cannot be costed to the job that
caused it otherwise. The asymmetric tail is a consequence the model handles for free, not the
reason for it. Anyone revisiting OQ-007 on cost grounds should re-argue it against labour costing,
not against this 2.77%.
